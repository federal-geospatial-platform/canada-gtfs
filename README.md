...
Code pour générer les données géospatiales des agences de transport en commun au Canada, avec des informations sur les emplacements des arrêts, les emplacements des lignes, les types des lignes, le niveau de service, l'accès aux fauteuils roulants, l'accès aux vélos, etc. Pour produire de [données de Systèmes de transport en commun canadiens](https://open.canada.ca/data/fr/dataset/b8241e15-2872-4a63-9d36-3083d03e8474) hébergées sur la Plateforme géospatiale fédérale. Les shapefiles sont également fournis dans le référentiel.
...
Cloner le référentiel pour créer des copies locales des fichiers. Exécutez <code>/data/1-process_gtfs.R</code> pour augmenter la mémoire locale, charger les liens vers les données GTFS et créer les fonctions pour télécharger les données. Ensuite, exécutez <code>download.gtfs.canada()</code> pour parcourir les liens vers les données GTFS et traiter les informations pertinentes. Cela produira un fichier <code>.fst</code>, qui est un fichier de stockage de données rapide à télécharger. <code>/data/1-process_gtfs.R</code> contient également une fonction <code>map.gtfs.system()</code> pour mapper les systèmes GTFS à l'aide des noms de système de transit fournis dans le <code>gtfs_sources.xlsx</code > dossier. Pour produire des shapefiles, exécutez le fichier <code>/data/2-export_gtfs.R</code>. En raison de la taille des données, l'exécution des deux fichiers prend environ trente minutes chacun.

Pour accéder aux données sans exécuter le code, visitez les [données de Systèmes de transport en commun canadiens](https://open.canada.ca/data/fr/dataset/b8241e15-2872-4a63-9d36-3083d03e8474) hébergées sur la Plateforme géospatiale fédérale. Notez que cette méthode peut ne pas fournir les informations les plus récentes des systèmes de transport en commun.
